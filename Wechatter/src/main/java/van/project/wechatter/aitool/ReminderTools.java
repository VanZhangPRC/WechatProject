...
public class ReminderTools {
    ...
    @Tool(name = "addReminder", description = """
            ...
            如果可以根据现有工具辅助完成，则将 content 内容填写为可以用于指导AI完成数据采集的 skill 描述:
            1. skill 内容为指导LLM完成数据采集的工作
            2. 注意分工边界，LLM辅助完成如获取数据结果等任务，不包含把结果推送给用户的动作
            3. skill 内容用于指导LLM完成工作，比如如何调用tool-calling获取数据、整理数据等
            4. skill 内容长度不超过900，
